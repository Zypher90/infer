...
import { withRetry } from './apiUtils.js';
...
const confidenceSchema = {
  type: Type.OBJECT,
  properties: {
    isConfident: { type: Type.BOOLEAN, description: 'True only if the item has a clear owner AND a clear, concrete task - safe to act on automatically.' },
...
export async function judgeConfidence(item) {
  return withRetry(async () => {
    const prompt =  `
        Judge whether this action item is specific and clear enough to automatically create a real ticket for, without human review.

        Item: text="${item.text}", owner="${item.owner}", deadline="${item.deadline}"

        This item is confident ONLY if BOTH are true:
